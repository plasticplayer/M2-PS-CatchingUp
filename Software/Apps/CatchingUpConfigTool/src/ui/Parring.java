...
public class Parring extends JFrame {
	...
	public Parring( Recorder rec ) {
		if ( rec.getStatus() != RecorderStatus.CONNECTED ){
			new MessageBox("Module d'enregistrement non connecté").setVisible(true);
			dispose();
		}
		else if ( rec.isRecording() ){
			new MessageBox("Module actuelement en enregistrement").setVisible(true);
			dispose();
		}
		else {
			frame = this;
			...
			final JLabel text = new JLabel( "Pour appairer, cliquer sur Appairer" );
			final JLabel text2 = new JLabel( "Presser pendant 5 secondes le bouton" );
			...
			btnCalibrer.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					String image = RecorderDAOImpl._instance.getImage(_recorder);
					if ( image.isEmpty() )
						new MessageBox("Erreur d'acquisition de l'image").setVisible(true);
					else {
						try {
							BufferedWriter writer = new BufferedWriter( new FileWriter( "/Volumes/640Go/tmp/a.jpg"));
							writer.write(image);
							writer.close( );
									...
									.addGroup(gl_contentPane.createSequentialGroup()
											...
											.addGroup(gl_contentPane.createSequentialGroup()
													...
													.addGroup(Alignment.LEADING, gl_contentPane.createSequentialGroup()
															.addGap(18)
															.addComponent(text2, GroupLayout.PREFERRED_SIZE, 370, GroupLayout.PREFERRED_SIZE)
			...
			gl_contentPane.setVerticalGroup(
					...
					.addGroup(gl_contentPane.createSequentialGroup()
							.addContainerGap()
							.addComponent(text, GroupLayout.PREFERRED_SIZE, 40, GroupLayout.PREFERRED_SIZE)
