...
public:
    ...
    bool SetDataPortRange (unsigned short int usStart,
                           unsigned short int uiLen);
...
private:
     ...
     *          on error    NULL.
     */
    CFtpServer::CClientEntry * AddClient (SOCKET Sock,
...
public:
     ...
     */
    void
    SetMaxNumberOfClient (unsigned int uiMax)
    {
     ...
     */
    bool
    SetExtraCommand (unsigned char dExtraCmd);
...
private:
    ...
    */
    ...
    char
    szLogin[MaxLoginLen + 1];

    char
    szPassword[MaxPasswordLen + 1];

    char
    szStartDirectory[MaxRootPathLen + 1];
...
private:
    ...
    char
    szWorkingDir[MAX_PATH + 3 + 1];

    char
    szRenameFromPath[MAX_PATH + 1];
    ...
    char
    sCmdBuffer[MAX_COMMAND_LINE_LEN + 1];
