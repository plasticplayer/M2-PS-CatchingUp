...
bool
CFtpServer::SetDataPortRange (unsigned short int usStart,
                              unsigned short int usLen)
{
    if (usLen != 0 && usStart > 0 && (usStart + usLen) <= 65535)
    {
        ...
        FtpServerLock.Leave ();
        Logger::Write (Logger::INFO, "Setting FTP port range");
        return true;
    }
    return false;
}
...
bool
CFtpServer::GetDataPortRange (unsigned short int *usStart,
                              unsigned short int *usLen)
{
    if (usStart && usLen)
    {
...
bool
CFtpServer::StartListening (unsigned long ulAddr, unsigned short int usPort)
{
    ...
    FtpServerLock.Enter ();
    {
        ...
        if (ListeningSock != INVALID_SOCKET)
        {
            ...
            if (setsockopt
                    (ListeningSock, SOL_SOCKET, SO_REUSEADDR, (char *) &on,
                     sizeof (on)) != SOCKET_ERROR)
            {
#endif
                if (bind
                        (ListeningSock, (struct sockaddr *) &sin,
                         sizeof (struct sockaddr_in)) != SOCKET_ERROR)
                {
                    if (listen (ListeningSock, 0) != SOCKET_ERROR)
                    {
                        Logger::Write (Logger::INFO, "Starting listening OK");
                    ...
                    {
                        Logger::Write (Logger::ERROR, strerror (errno));
                        //perror("Listen Error : ");
                        if (ListeningSock != INVALID_SOCKET)
...
#endif
{
    ...
    for (;;)
    {
        ...
        if (select (ListeningSock + 1, &fdRead, NULL, NULL, NULL) ==
                SOCKET_ERROR)
            break;
        ...
        if (TempSock != INVALID_SOCKET)
        {
            CFtpServer::CClientEntry * pClient =
                pFtpServer->AddClient (TempSock, &Sin);
            if (pClient != NULL)
            {
#ifdef WIN32
                pClient->hClientThread = (HANDLE) _beginthreadex (NULL, 0,
                                         pClient->Shell,
                                         pClient, 0,
                                         &pClient->uClientThreadID);
                if (pClient->hClientThread == 0)
                {
#else
                if (pthread_create
                        (&pClient->ClientThreadID, &pFtpServer->m_pattrClient,
...
char *
CFtpServer::CClientEntry::BuildPath (char *pszAskedPath,
                                     char **pszVirtualPath /* = NULL */ )
{
    ...
    char *
    pszBuffer = new char[MAX_PATH + 4];
    if (pszBuffer)
    {

        char *
        pszVirtualP = BuildVirtualPath (pszAskedPath);
        if (pszVirtualP)
        {
        ...
        if (a[3] == '/' || a[3] == '\0')
        {
            ...
            else
            {
                ...
                while (b != pszPath)
                {
                    ...
                    if (*b == '/')
                    {
                        if (b == pszPath || (b == (pszPath + 2) && isalpha (pszPath[0]) && pszPath[1] == ':')	// C:
                           )
                        {
                            if (a[3] == '/')
                            {   // e.g. '/foo/../' or 'C:/lol/../'
...
CFtpServer::CUserEntry * CFtpServer::AddUser (const char *pszLogin,
        const char *pszPass,
        const char *pszStartDir)
{
            ...
            && (!pszPass || strlen (pszPass) <= MaxPasswordLen))
    {
        ...
        char *
        pszSDEx = strdup (pszStartDir);
        if (!pszSDEx)
            return NULL;
        SimplifyPath (pszSDEx);
        struct stat
                st;			// Verify that the StartPath exist, and is a directory
...
CFtpServer::CClientEntry * CFtpServer::AddClient (SOCKET Sock,
        struct sockaddr_in * Sin)
{
    if (Sock != INVALID_SOCKET && &Sin)
    {
        ...
        if (pClient)
        {
            ...
            pClient->ulServerIP =
                (((struct sockaddr_in *) &(ServerSin))->sin_addr).s_addr;
            pClient->ulClientIP =
...
{
    if ((ucPriv &
            ~(READFILE | WRITEFILE | DELETEFILE | LIST | CREATEDIR | DELETEDIR)) ==
            0)
    {
        ucPrivileges = ucPriv;
        return true;
...
CFtpServer::SearchUserFromLogin (const char *pszLogin)
{
    if (pszLogin)
    {
        ...
        while (pSearchUser)
        {
            if (pSearchUser->szLogin
                    && !strcmp (pszLogin, pSearchUser->szLogin))
            {
                break;
            }
...
{
    return (bIsLogged && pUser
            && (ucPriv & pUser->ucPrivileges) == ucPriv) ? true : false;
}
...
{   // Main Loop
        ...
        else if (nCmd == CMD_PASS)
        {
            ...
            else
            {
                ...
                {
                    if (pClient->pUser && pClient->pUser->bIsEnabled)
                    {   // User is valid & enabled
                        if ((pszCmdArg && !strcmp (pszCmdArg, pClient->pUser->szPassword)) || !*pClient->pUser->szPassword)	// Password is valid.
                        {
        ...
        else if (nCmd == CMD_SITE)
        {
            ...
            char *
            pszSiteArg = pszCmdArg;
            if (pszCmdArg)
            {
                unsigned char
                ucExtraCmd = pClient->pUser->GetExtraCommand ();
        ...
        else if (nCmd == CMD_CLNT)
        {
            ...
            if (pszCmdArg)
            {
                pFtpServer->OnClientEventCb (CLIENT_SOFTWARE, pClient,
                                             pszCmdArg);
        ...
        else if (nCmd == CMD_STRU)
        {
            ...
            if (pszCmdArg)
            {
                if (!strcmp (pszCmdArg, "F"))
                {
                    ...
                    pClient->SendReply
                    ("504 STRU failled. Parametre not implemented");
            }
            else
                pClient->SendReply ("501 Invalid number of arguments.");
            continue;
        ...
        else if (nCmd == CMD_MODE)
        {
            ...
            if (pszCmdArg)
            {
                ...
                else if (!strcmp (pszCmdArg, "Z"))
                {
                    ...
                    pClient->SendReply2 ("504 \"%s\": Unsupported transfer MODE.",
                                         pszCmdArg);
            }
            else
                pClient->SendReply ("501 Invalid number of arguments.");
        ...
        else if (nCmd == CMD_PORT)
        {
            ...
            if (pClient->eStatus == WAITING && pszCmdArg
                    && sscanf (pszCmdArg, "%u,%u,%u,%u,%u,%u", &iIp1, &iIp2, &iIp3,
                               &iIp4, &iPort1, &iPort2) == 6 && iIp1 <= 255
                    && iIp2 <= 255 && iIp3 <= 255 && iIp4 <= 255 && iPort1 <= 255
                    && iPort2 <= 255 && (iIp1 | iIp2 | iIp3 | iIp4) != 0
                    && (iPort1 | iPort2) != 0)
            {
                         ...
                         || pClient->ulClientIP == inet_addr ("127.0.0.1")))
                {
                    ...
                    pClient->usDataPort =
                        (unsigned short) ((iPort1 * 256) + iPort2);
        ...
        else if (nCmd == CMD_PASV)
        {
...
#ifndef WIN32
                        && setsockopt (pClient->DataSock, SOL_SOCKET, SO_REUSEADDR,
                   ...
                   )
            {
                    ...
                    unsigned short
                    usTriedPort = usFirstTry;
                    for (;;)
                    {
                        sin.sin_port = htons (usTriedPort);
                        if (!bind
                                (pClient->DataSock, (struct sockaddr *) &sin,
                    ...
                    unsigned long
                    ulIp = ntohl (pClient->ulServerIP);
                    pClient->SendReply2
                    ("227 Entering Passive Mode (%lu,%lu,%lu,%lu,%u,%u)",
                     (ulIp >> 24) & 255, (ulIp >> 16) & 255,
                     (ulIp >> 8) & 255, ulIp & 255, pClient->usDataPort / 256,
        ...
        else if (nCmd == CMD_LIST || nCmd == CMD_NLST || nCmd == CMD_STAT)
        {
            ...
            {
                pClient->SendReply
                ("211 :: CFtpServer / Browser FTP Server:: thebrowser@gmail.com");
                continue;
            }
            ...
            memset (&pClient->CurrentTransfer, 0x0,
                    sizeof (pClient->CurrentTransfer));

            char *
            pszArg = pszCmdArg;	// Extract arguments
            if (pszCmdArg)
            {
                while (*pszArg == '-')
                {
        ...
        else if (nCmd == CMD_CWD || nCmd == CMD_XCWD)
        {
            ...
            if (pszCmdArg)
            {
                ...
                if (pszPath && stat (pszPath, &st) == 0 && S_ISDIR (st.st_mode))
                {
                    ...
                    pFtpServer->OnClientEventCb (CLIENT_CHANGE_DIR, pClient,
                                                 pszPath);
                }
                else
        ...
        else if (nCmd == CMD_PWD || nCmd == CMD_XPWD)
        {
            ...
            pClient->SendReply2 ("257 \"%s\" is current directory.",
                                 pClient->szWorkingDir);
            continue;
        ...
        else if (nCmd == CMD_ABOR)
        {
            ...
            if (pClient->eStatus != WAITING)
            {
                pClient->ResetDataConnection ();
                pClient->SendReply
                ("426 Previous command has been finished abnormally.");
            }
        ...
        else if (nCmd == CMD_RETR)
        {
            ...
            if (pszCmdArg)
            {
                ...
                if (pszPath && stat (pszPath, &st) == 0 && S_ISREG (st.st_mode))
                {
                    ...
                    pClient->CurrentTransfer.eDataMode = pClient->eDataMode;
#endif
                    pFtpServer->OnClientEventCb (CLIENT_DOWNLOAD, pClient,
                                                 pszPath);
                    ...
                    if (pthread_create
                            (&pClient->CurrentTransfer.TransferThreadID,
                             &pFtpServer->m_pattrTransfer, RetrieveThread,
                             pClient) != 0)
                    {
#endif
                        pClient->ResetDataConnection ();
        ...
        else if (nCmd == CMD_STOR || nCmd == CMD_APPE || nCmd == CMD_STOU)
        {
            ...
            if (pszCmdArg || nCmd == CMD_STOU)
            {
                ...
                if (pszPath)
                {
                    ...
                    if (pthread_create
                            (&pClient->CurrentTransfer.TransferThreadID,
                             &pFtpServer->m_pattrTransfer, StoreThread,
                             pClient) != 0)
                    {
#endif
                        pClient->ResetDataConnection ();
        ...
        else if (nCmd == CMD_RNTO)
        {
            ...
            if (pszCmdArg)
            {
                if (pClient->szRenameFromPath)
                {
                    pszPath = pClient->BuildPath (pszCmdArg);
                    if (pszPath
                            && rename (pClient->szRenameFromPath, pszPath) == 0)
                    {
...
void
CFtpServer::CClientEntry::LogOut ()
{
    if (pUser)
    {
        --pUser->uiNumberOfClient;
...
bool
CFtpServer::CClientEntry::SendReply (const char *pszReply,
                                     bool bNoNeedToAlloc /* =0 */ )
{
    if (pszReply)
    {
        bool bReturn = false;
        int
        nLen = strlen (pszReply);
        char *
        pszBuffer;
        if (!bNoNeedToAlloc)
        {
...
bool CFtpServer::CClientEntry::SendReply2 (const char *pszList, ...)
{
    ...
    {
        char *
        pszBuffer = new char[CFTPSERVER_REPLY_MAX_LEN];
        if (pszBuffer)
        {
            va_list args;
    ...
    for (;;)
{
        ...
        if (!FD_ISSET (CtrlSock, &fdRead))
        {
                        ...
                        && dt >= pFtpServer->GetNoTransferTimeout ()))
            {
                ...
                pFtpServer->OnClientEventCb (IsLogged ()? NO_TRANSFER_TIMEOUT :
                                             NO_LOGIN_TIMEOUT, this);
                return false;
            }
            else
                continue;
        ...
        int
        Err = recv (CtrlSock, sCmdBuffer + iCmdRecvdLen, iRemainingCmdLen, 0);
        if (Err > 0)
        {
...
int
CFtpServer::CClientEntry::ParseLine ()
{
    // Separate the Cmd and the Arguments
    char *
    pszSpace = strchr (sCmdBuffer, ' ');
    if (pszSpace)
    {
        pszSpace[0] = '\0';
...
bool CFtpServer::CClientEntry::OpenDataConnection (int nCmd)
{
    ...
    int
    iBufSize = pFtpServer->GetTransferSocketBufferSize ();
            ...
            sizeof (int)))
    {
        ...
        if (eDataConnection == PASV)
        {
            ...
            if (select (DataSock + 1, &fdRead, NULL, NULL, &tv) > 0
                    && FD_ISSET (DataSock, &fdRead))
        {
        ...
        {   // eDataConnection == PORT
            ...
            int
            on = 1;		// Here the strange behaviour of SO_REUSEADDR under win32 is welcome.
#ifdef SO_REUSEPORT
            (void) setsockopt (DataSock, SOL_SOCKET, SO_REUSEPORT, (char *) &on,
                               sizeof on);
#else
            ...
            if (bind
                    (DataSock, (struct sockaddr *) &BindSin,
                     sizeof (struct sockaddr_in)) != SOCKET_ERROR)
        {
                ...
                if (connect
                        (DataSock, (struct sockaddr *) &ConnectSin,
...
bool
CFtpServer::CClientEntry::ResetDataConnection (bool bSyncWait /* =true */ )
{
    ClientLock.Enter ();
    {
        if (DataSock != INVALID_SOCKET)
...
bool CFtpServer::CClientEntry::InitZlib (DataTransfer_t * pTransfer)
{
    ...
    CFtpServer *
    pFtpServer = pTransfer->pClient->pFtpServer;
...
#endif
{
    ...
    struct DataTransfer_t *
            pTransfer = &pClient->CurrentTransfer;
                    ...
                    if (len >= 0)
                    {
                        ...
                        if (pTransfer->eDataMode == ZLIB)
                        {
                            ...
                            do
                            {
                                ...
                                if (pTransfer->zStream.avail_out == 0
                                        || nRet == Z_STREAM_END)
                                {
                            ...
                            if (len > 0)
                            {
                                ...
                                if (ps == pe)
                                {
                                    if (!pClient->SafeWrite
                                            (hFile, pBuffer, ps - pBuffer))
                                    {
                                        len = -1;
                                        break;
                                    }
                    ...
                    && (BlockSize = read (hFile, pBuffer, uiBufferSize)) > 0)
            {
                ...
                if (pTransfer->eDataMode == ZLIB)
                {
                    ...
                    do
                    {
                        ...
                        {
                            len =
                                send (pClient->DataSock, pOutBuffer,
                ...
                {
                    len =
                        send (pClient->DataSock, pBuffer, BlockSize,
                              MSG_NOSIGNAL);
                    if (len <= 0)
                        break;
                }
...
int
CFtpServer::CClientEntry::GetFileListLine (char *psLine, unsigned short mode,
#ifdef __USE_FILE_OFFSET64
        __int64 size,
#else
        long size,
#endif
        time_t mtime, const char *pszName,
        bool opt_F)
{
    ...
    static const char
    szListFile[] = "%c%c%c%c%c%c%c%c%c%c 1 user group %14"
#ifdef __USE_FILE_OFFSET64
#ifdef WIN32
                   "I64"
#else
    ...
    struct tm *
    t = gmtime ((time_t *) & mtime);	// UTC Time
    ...
    int
    iLineLen = sprintf (psLine, szListFile,
                        (S_ISDIR (mode)) ? 'd' : '-',
...
bool
CFtpServer::CClientEntry::AddToListBuffer (DataTransfer_t * pTransfer,
        char *pszListLine, int nLineLen,
        char *pBuffer,
        unsigned int *nBufferPos,
        unsigned int uiBufferSize)
{
    ...
    if (pTransfer->eDataMode == ZLIB)
    {
        ...
        int
        nFlush = pszListLine ? Z_NO_FLUSH : Z_FINISH;
        do
        {
            nRet = deflate (&pTransfer->zStream, nFlush);
            if (nRet == Z_STREAM_ERROR)
            ...
            if (pTransfer->zStream.avail_out == 0 || nRet == Z_STREAM_END)
            {
                *nBufferPos = uiBufferSize - pTransfer->zStream.avail_out;
                if (send
                        (pTransfer->SockList, pBuffer, *nBufferPos,
                         MSG_NOSIGNAL) <= 0)
                    return false;
                *nBufferPos = 0;
...
#endif
    {
        ...
        {
            int
            nBufferAvailable = uiBufferSize - *nBufferPos;
            int
            iCanCopyLen =
                (nLineLen <= nBufferAvailable) ? nLineLen : nBufferAvailable;
            memcpy ((pBuffer + *nBufferPos), pszListLine, iCanCopyLen);
            *nBufferPos += iCanCopyLen;
            if (*nBufferPos == uiBufferSize)
            {
                if (send
                        (pTransfer->SockList, pBuffer, uiBufferSize,
                         MSG_NOSIGNAL) <= 0)
                    return false;
                *nBufferPos = 0;
                if (iCanCopyLen < nLineLen)
                {
                    memcpy (pBuffer, pszListLine + iCanCopyLen,
                            nLineLen - iCanCopyLen);
...
#endif
{
    ...
    iFileLineLen = 0;
    char *
    psFileLine = new char[CFTPSERVER_LIST_MAX_LINE_LEN];
    ...
    if (psFileLine)
{
        ...
        if (pTransfer->opt_d || !S_ISDIR (st->st_mode))
        {
            ...
            char *
            pszName = strrchr (pTransfer->szPath, '/');
            iFileLineLen =
                pClient->GetFileListLine (psFileLine, st->st_mode, st->st_size,
                                          st->st_mtime,
                                          ((pszName
                                            && pszName[1]) ? pszName + 1 : "."),
                                          pTransfer->opt_F);
        ...
        else
        {
            ...
            CEnumFileInfo *
            fi = new CFtpServer::CEnumFileInfo;
            unsigned int
            uiBufferSize = pFtpServer->GetTransferBufferSize ();
            char *
            pBuffer = new char[uiBufferSize];
            ...
            if (fi->FindFirst (pTransfer->szPath))
            {
                do
                {
                    if (pClient->CurrentTransfer.nCmd == CMD_NLST)
                    {
                        ...
                        {
                            if (!pClient->AddToListBuffer
                                    (pTransfer, fi->pszName, strlen (fi->pszName),
                                     pBuffer, &nBufferPos, uiBufferSize))
                                break;
                        }
                    ...
                    else if (fi->pszName[0] != '.' || pTransfer->opt_a)
                    {
                        ...
                        iFileLineLen =
                            pClient->GetFileListLine (psFileLine, fi->mode,
                                                      fi->size, fi->mtime,
                                                      fi->pszName,
                                                      pTransfer->opt_F);

                        if (!pClient->AddToListBuffer (pTransfer, psFileLine,
                                                       iFileLineLen, pBuffer,
                                                       &nBufferPos,
                                                       uiBufferSize))
                            break;
                ...
                // Flush the buffer
                pClient->AddToListBuffer (pTransfer, NULL, 0, pBuffer,
...
bool CFtpServer::CEnumFileInfo::FindNext ()
{
    ...
    {
        int
        iDirPathLen = strlen (szDirPath);
        int
        iFileNameLen = strlen (c_file.name);
        if (iDirPathLen + iFileNameLen >= MAX_PATH)
            return false;
        sprintf (szFullPath, "%s%s%s", szDirPath,
                 (szDirPath[iDirPathLen - 1] == '/') ? "" : "/", c_file.name);
            ...
            && dir_entry_result != NULL)
{
        ...
        iDirPathLen = strlen (szDirPath);
        int
        iFileNameLen = strlen (dir_entry.d_name);
        if (iDirPathLen + iFileNameLen >= MAX_PATH)
            return false;
